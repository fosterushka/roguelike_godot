...
func run() -> void:
	...
	for level in range(1,5):
		...
		check(Rules.power_at(level) <= 100 and (Rules.power_at(level) == 100) == (level == 4), "Full power is reached only at final tier")
	...
	check(not progression.get_shop_state().radar_upgrade.enabled and model.player.radar_range == 260, "Maximum tier caps at100percent260m")
