...
func boss() -> void:
	...
	close(parent.components[0].position.y, 6.3 * 1.32, "source missileanchorheight")
	close(parent.components[1].position.x, 1.5 * 1.32, "source gunanchoroffset")
	...
	close(parent.components[2].position.z, 35.0 + 3.55 * 1.32, "componentfollowsparentyaw")
