...
static func interpolate(previous: Dictionary, current: Dictionary, fraction: float) -> Dictionary:
	...
	for field in ["height", "pitch", "roll"]:
		pose.suspension[field] = lerpf(float(previous.suspension[field]), float(current.suspension[field]), alpha)
	for index in 4:
...
static func transform(pose: Dictionary) -> Transform3D:
	return Transform3D((Basis(Vector3.UP, pose.heading) * Basis(Vector3.BACK, float(pose.get("wind_roll", 0.0))) * Suspension.body_basis(pose.suspension)).scaled(Vector3.ONE * float(pose.scale)), pose.position)
